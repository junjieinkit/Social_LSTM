...
class Model():
    ...
    def __init__(self, args, infer=False):
        ...
        self.input_data = tf.placeholder(tf.float32, [None, args.seq_length, 2])    #tf.placeholder(dtype，shape=None,name=None)
                                                                                    #batch_size自动填充，2是x，y
        ...
        with tf.variable_scope("coordinate_embedding"):
            ...
            #  Embed the 2D coordinates into embedding_size dimensions       #QUESTION: why does it need a embedding layer?
            #  TODO: (improve) For now assume embedding_size = rnn_size
            embedding_w = tf.get_variable("embedding_w", [2, args.embedding_size])  #tf.get_variable(name,shape,......)
                                                                                    #embedding_size:Default = 128
            embedding_b = tf.get_variable("embedding_b", [args.embedding_size])    #QUESTION:没有指定initializer和trainalbe，所以他是否需要初始化和训练？

        # Output linear layer
        with tf.variable_scope("rnnlm"):

            #(Default)rnn_size=128:size of RNN hidden state
        ...
        # Nella versione di tf<1.0 era : inputs = tf.split(1, args.seq_length, self.input_data) but changed because the order of parameters is changed
        #input_data.shape=[None, args.seq_length, 2]
        inputs = tf.split(self.input_data, args.seq_length, 1)    #tf.split(value,num_or_size_splits,axis=0,....)
        #将序列按时间步拆开。  如：  inputs=[[batch_size,1,[x0,y0]],[batch_size,1,[x1,y1]],.........]????     #QUESTION:在看代码的时候，我怎么可以随时查看某个变量的shape？



        # Get a list of 2D tensors. Each of size numPoints x 2
        inputs = [tf.squeeze(input_, [1]) for input_ in inputs]    #删除input_中size为1的维度。
        ...
        for x in inputs:
            ...
            # Nella versione di tf<1.0 era : outputs, last_state = tf.nn.seq2seq.rnn_decoder(embedded_inputs, self.initial_state, cell, loop_function=None, scope="rnnlm")
        ...
        #  Nella versione di tf<1.0 era : output = tf.reshape(tf.concat(1, outputs), [-1, args.rnn_size])
        output = tf.reshape(tf.concat(outputs, 1), [-1, args.rnn_size])    #tf.concat（values，axis,name=''），沿着axis堆叠values
        ...
        # Nella versione di tf<1.0 era : [x_data, y_data] = tf.split(1, 2, flat_target_data)
        [x_data, y_data] = tf.split(flat_target_data, 2, 1)

        def tf_2d_normal(x, y, mux, muy, sx, sy, rho):  #计算公式（4）概率P
            '''
    ...
    def sample(self, sess, traj, true_traj, num=10):
        ...
        Params:[state] = sess.run([self.final_state], feed)
        ...
        def sample_gaussian_2d(mux, muy, sx, sy, rho):
            ...
            x = np.random.multivariate_normal(mean, cov, 1)   #x.shape=(1,2)
            return x[0][0], x[0][1]
        ...
        for pos in traj[:-1]:   #一条轨迹中，从第一个点到倒数第二个点。最后一个点不算
            ...
            [state] = sess.run([self.final_state], feed)  #sess.run(被求的对象, feed_dict={x: batch_xs, y_: batch_ys})
            #QUESTION：为什么【state】要用【】
        ...
        # Construct the input data tensor for the last point  #QUESTION:为什么要取出最后一点？
        ...
        for t in range(num):    '''num: Number of time-steps into the future to be predicted'''
            #Create the feed dict
        ...
        return ret  #输出一条完整的轨迹。包括预测的部分的轨迹
