...
def main():
    ...
    saver.restore(sess, ckpt.model_checkpoint_path)     #QUESTION：此时sess中储存了已经训练好的网络？
    ...
    total_error = 0.
    counter = 0.
    for b in range(data_loader.num_batches):   #注意区分batch_size和num_batches，
                                                #batch_size：有多少个batch
                                                #num_batch：batc里面有多少个trajectory
        # Get the source, target data for the next batch
        x, y = data_loader.next_batch()      #QUESTION： y is not used
        ...
        complete_traj = model.sample(sess, obs_traj, x[0], num=sample_args.pred_length)  #QUESTION：用已经训练好的网络sess来预测轨迹？
